...
def supported_distributions(distname=None):
    ...
    if distname is not None:
        ...
        try:
            ...
            print(
                'Please specify a supported distribution. Type `?supported_distributions`')
...
class skde(object):
    ...
    def __init__(self, data, mirror=False, **kwds):
        ...
        if kwds is None:
            ...
            if self.mirror:
                self.kde_object = KernelDensity(
                    **kwds).fit(np.vstack([-data.reshape(-1, 1), data.reshape(-1, 1)]))
            else:
                self.kde_object = KernelDensity(
                    **kwds).fit(data.reshape(-1, 1))
        try:
            self.d = data.shape[1]
        except IndexError:
            self.d = 1
    ...
    def pdf(self, eval_points):
        ...
        if self.mirror:
            ...
            try:
                ...
                p = np.exp(self.kde_object.score_samples(
                    eval_points.reshape(-1, 1)))
        return p
...
class parametric_dist(object):
    ...
    def rvs(self, size=None, seed=5397):
        ...
        output = np.concatenate([D[dist].rvs(size=(n, 1), random_state=(
            1+seed+int(dist))).reshape(-1, 1) for dist in D.keys()], axis=1)
        return output
    ...
    def pdf(self, eval_points):
        ...
        try:
            ...
            if len(D) != dim:
                raise(IndexError(
                    "Could not infer dimensions. `eval_points` has the wrong shape."))
        try:
            n = size[0]
        except IndexError:
            n = 1
