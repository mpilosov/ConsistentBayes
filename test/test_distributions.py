...
class TestDistributions(object):
    ...
    def teardown(self):
        # remove it from memory in preparation for the next test.
        self.S = None
        self.P = None

    def test_supp_dist_norm(self):
        print('========== testing distributions.supported_distributions for Normal ==========\n')
        assert_equals(type(distributions.supported_distributions(
            'Normal')), _continuous_distns.norm_gen)
        assert_equals(type(distributions.supported_distributions(
            'Gauss')), _continuous_distns.norm_gen)
        assert_equals(type(distributions.supported_distributions(
            'Gaussian')), _continuous_distns.norm_gen)
        assert_equals(type(distributions.supported_distributions(
            'Norm')), _continuous_distns.norm_gen)
        assert_equals(type(distributions.supported_distributions(
            'N')), _continuous_distns.norm_gen)

    def test_supp_dist_uni(self):
        print('========== testing distributions.supported_distributions for Uniform ==========\n')
        assert_equals(type(distributions.supported_distributions(
            'Uniform')), _continuous_distns.uniform_gen)
        assert_equals(type(distributions.supported_distributions(
            'U')), _continuous_distns.uniform_gen)
        assert_equals(type(distributions.supported_distributions(
            'Uni')), _continuous_distns.uniform_gen)
