...
for dim in range(1,6):
    ...
    S = sample.sample_set(size=(num_samples, dim)) # ... Alternatively, the three lines above
    S.set_dist('uniform', 0*ones, 1*ones) # uniform priors in all directions. 'normal' also available.
    S.generate_samples() # generate samples, store them in the sample_set object.
    P = sample.map_samples_and_create_problem(S, model) # map the samples, create new `sample_set` for outputs, put them together into a `problem_set` object.
    P.compute_pushforward_dist() # gaussian_kde by default on the data space.
    P.set_observed_dist('uniform', 0.5*ones, 1*ones) # define your observed distribution.
